...
export default async function PhilosophyPortal({ params }: PageProps) {
  ...
  return (
    <main className="bg-[#06070b] text-[#dae2fd] min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden font-sans">
      ...
      <div className="max-w-xl w-full border border-[#171f33] bg-[#0b1326]/60 backdrop-blur-md p-8 md:p-12 rounded-2xl shadow-2xl space-y-8 z-10 text-center transition-all duration-700">
        <div className="space-y-2">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#131b2e] border border-[#212d4a] rounded-full text-xs text-cyan-400 font-mono">
            {isAuthorized ? (
              <>
                <Sparkles className="w-3.5 h-3.5 text-[#e8b3ff]" /> System
                Online
              </>
            ) : (
              <>
          ...
          <p className="text-[#958ea0] text-sm mt-4 leading-relaxed">
            {isAuthorized
        ...
        {isAuthorized && (
          <div className="pt-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <Link
              ...
              className="inline-flex items-center gap-2 px-8 py-3 bg-[#9d4edd] hover:bg-[#b05cf0] text-white font-medium rounded-lg transition-all shadow-[0_0_20px_rgba(157,78,221,0.3)] hover:shadow-[0_0_30px_rgba(157,78,221,0.5)]"
            >
              Enter Dashboard
