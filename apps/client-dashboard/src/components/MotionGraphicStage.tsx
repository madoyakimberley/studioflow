...
// Proportional, tight letter-spacing calibrated for standard typography text tracking
...
export default function MotionGraphicStage() {
  ...
  const animStateRef = useRef({
    ...
    inkDrips: LETTER_OFFSETS.map((offset, i) =>
      i === 0
        ? [
            { xOffset: -4, length: 0, targetLength: 28, width: 2.2 },
            { xOffset: 4, length: 0, targetLength: 35, width: 1.8 },
          ]
        : [],
    ) as InkDrip[][],
  });
  ...
  useEffect(() => {
    ...
    // HIGH-DPI SCALING FIX: Multiplies physical canvas dimensions by pixel ratio for perfect rendering
    ...
    const renderLoop = (timestamp: number) => {
      ...
      if (isPlaying) {
        if (s.state === "INTRO") {
          ...
          s.cooldownProgress += elapsed / 2000;
      ...
      if (
        ...
        s.trackingFactor = 1.0 + overallBackwardT * 0.12;
      } else if (s.state === "RISE_BACK" || s.state === "COOLDOWN") {
        s.trackingFactor = 1.12;
      }

      // Ball Radius Dynamics
      if (s.state === "INTRO" || s.state === "BALL_DROP") {
        s.ballRadius = 20;
      } else if (s.state === "BOUNCE_FORWARD") {
        const rawT = s.bounceIndex + s.bounceProgress;
        s.ballRadius = 20 - (rawT / 9.0) * 9.0;
      } else if (s.state === "BOUNCE_BACKWARD") {
        s.ballRadius = 11;
      } else if (s.state === "RISE_BACK") {
        s.ballRadius = 11 + s.riseProgress * 9.0;
      } else if (s.state === "COOLDOWN") {
        s.ballRadius = 20;
      }

      // 2. Ball Positioning Mechanics
      let bx = 428;
      let by = 142;
      const bHeight = 55;
      ...
      s.inkDrips.forEach((drips, idx) => {
        ...
        drips.forEach((d) => {
          if (isFilled)
            d.length = Math.min(d.targetLength, d.length + elapsed * 0.08);
          else d.length = Math.max(0, d.length - elapsed * 0.16);
        });
      });
      ...
      LETTER_OFFSETS.forEach((offset, idx) => {
        ...
        let textOpacity = 1.0;
        if (s.state === "INTRO") textOpacity = 0.0;
        else if (s.state === "BALL_DROP") textOpacity = s.dropProgress;
        else if (s.state === "RISE_BACK") textOpacity = 1.0 - s.riseProgress;
        else if (s.state === "COOLDOWN") textOpacity = 0.0;
      ...
      // Draw standard text typography layer
      ...
      // FIXED: The ball element now stays alive and renders across ALL active animation timeline states!
      ctx.save();
